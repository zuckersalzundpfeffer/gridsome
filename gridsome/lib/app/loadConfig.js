...
module.exports = async (context, options = {}) => {
  ...
  config.pathPrefix = normalizePathPrefix(
    config.mode === "production" ? localConfig.pathPrefix : ""
  );
  ...
  config.transpileDependencies = Array.isArray(
    localConfig.transpileDependencies
  )
  ...
  config.cacheBusting =
    typeof localConfig.cacheBusting === "boolean"
      ? localConfig.cacheBusting
      : true;

  config.catchLinks =
    typeof localConfig.catchLinks === "boolean" ? localConfig.catchLinks : true;
...
function resolvePkg(context) {
  ...
  if (
    !dependencies.includes("@zuckersalzundpfeffer/gridsome") &&
...
const template = Joi.object()
  ...
  .keys({
    ...
    path: Joi.alternatives([
      Joi.string().regex(
        /^\//,
        "Template string paths must begin with a slash"
      ),
      Joi.func(),
    ]).required(),
...
const permalinksSchema = Joi.object()
  ...
  .keys({
    trailingSlash: Joi.boolean().valid(true, false, "always").default(true),
...
function resolvePluginEntries(id, context) {
  ...
  if (typeof id === "function") {
    ...
    deprecate(
      `The ~ alias for plugin paths is deprecated. Use a relative path instead.`,
      {
  ...
  if (fs.existsSync(dirName) && fs.lstatSync(dirName).isFile()) {
    return Object.freeze({
      clientEntry: null,
      serverEntry: dirName,
    });
...
function normalizeImages(config = {}) {
  ...
  if (typeof config.defaultBlur !== "undefined") {
    ...
    deprecate(
      `The images.defaultBlur option has moved to images.placeholder.defaultBlur.`,
      {
        customCaller: ["gridsome.config.js"],
      }
    );
          ...
          .try([
            Joi.object()
              ...
              .keys({
                ...
                defaultBlur: Joi.number()
                  .min(0)
                  .default(defaultPlaceholder.defaultBlur),
              }),
            Joi.object()
              .label("Trace")
              .keys({
...
function normalizeIconsConfig(config = {}) {
  ...
  res.favicon =
    typeof icon.favicon === "string"
      ? { src: icon.favicon, sizes: faviconSizes }
      : Object.assign(
          {},
          { src: defaultIcon, sizes: faviconSizes },
          icon.favicon
        );

  res.touchicon =
    typeof icon.touchicon === "string"
      ? { src: icon.touchicon, sizes: touchiconSizes, precomposed: false }
      : Object.assign(
          {},
          { src: res.favicon.src, sizes: touchiconSizes, precomposed: false },
          icon.touchicon
        );
